...
import {
  readyStandardTrustFormation,
} from '@/trust-club/formation/trust-club-standard-trust-formation-ready.operation';
...
export async function POST(
  ...
  const authenticationRequest = {
    sourceReference:
      'standard-trust-formation-ready-route',
  };
  ...
  try {
    const result =
      await readyStandardTrustFormation({
    ...
    if (
      ...
      if (
        error.message.startsWith(
          'TRUST_CLUB_STANDARD_TRUST_FORMATION_NOT_READY:',
        )
      ) {
        return Response.json(
          {
    ...
    return Response.json(
      {
        status:
          'FORMATION_READY_TRANSITION_FAILED',
      },
      {
        status:
          500,
