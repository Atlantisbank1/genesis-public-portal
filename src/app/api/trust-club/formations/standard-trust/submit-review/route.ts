...
import {
  submitStandardTrustFormationForReview,
} from '@/trust-club/formation/trust-club-standard-trust-formation-submit-review.operation';
...
export async function POST(
  ...
  const authenticationRequest = {
    sourceReference:
      'standard-trust-formation-submit-review-route',
  };
  ...
  try {
    const result =
      await submitStandardTrustFormationForReview({
        applicationEntry: {
    ...
    if (
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'REVIEW_SUBMISSION_NOT_EXECUTED',
          },
          {
            status:
              409,
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'REVIEW_TRANSITION_INVARIANT_FAILED',
          },
          {
            status:
              500,
    ...
    return Response.json(
      {
        status:
          'FORMATION_REVIEW_SUBMISSION_FAILED',
      },
      {
        status:
          500,
