...
import {
  markStandardTrustFormationExternalPending,
} from '@/trust-club/formation/trust-club-standard-trust-formation-external-pending.operation';
...
export async function POST(
  ...
  const authenticationRequest = {
    sourceReference:
      'standard-trust-formation-external-pending-route',
  };
  ...
  if (
    ...
    return Response.json(
      {
        status:
          'ACTION_NOT_INTERNAL_COMPLETE_FOR_EXTERNAL_PENDING',
  ...
  try {
    ...
    const result =
      await markStandardTrustFormationExternalPending({
        applicationEntry: {
    ...
    if (
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'EXTERNAL_PENDING_TRANSITION_NOT_EXECUTED',
          },
          {
            status:
              409,
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'EXTERNAL_PENDING_TRANSITION_INVARIANT_FAILED',
          },
          {
            status:
              500,
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'EXTERNAL_PENDING_OUTCOME_NOT_RECORDED',
          },
          {
            status:
              409,
      ...
      if (
        ...
        return Response.json(
          {
            status:
              'EXTERNAL_PENDING_OUTCOME_INVARIANT_FAILED',
          },
          {
            status:
              500,
    ...
    return Response.json(
      {
        status:
          'FORMATION_EXTERNAL_PENDING_FAILED',
      },
      {
        status:
          500,
